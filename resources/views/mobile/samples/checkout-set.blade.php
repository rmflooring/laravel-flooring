<x-mobile-layout :title="'Check Out – ' . $sampleSet->set_id">

    {{-- Back --}}
    <a href="{{ route('mobile.samples.show', $sampleSet->set_id) }}"
       class="inline-flex items-center gap-1.5 text-sm text-blue-600 dark:text-blue-400 font-medium">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
        </svg>
        {{ $sampleSet->set_id }} – {{ $sampleSet->name ?? $sampleSet->productLine->name }}
    </a>
    ...
    <form method="POST" action="{{ route('mobile.sample-sets.checkout.store', $sampleSet->set_id) }}"
          x-data="checkoutForm()">
        @csrf
        ...
        <div x-show="type === 'staff'" x-cloak
            ...
            <div>
                ...
                <input type="text" name="notes"
                       placeholder="e.g. Job site, trade show, etc."
                       class="block w-full p-2.5 text-sm text-gray-900 bg-gray-50 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white dark:border-gray-600">
            </div>
        </div>
