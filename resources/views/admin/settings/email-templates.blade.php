    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">System Email Templates</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Manage templates for system notification emails (RFM alerts etc.).
            ...
            {{-- Tab nav --}}
            <div x-data="{ tab: '{{ request('tab', array_key_first(\App\Models\EmailTemplate::SYSTEM_TYPES)) }}' }">

                <div class="border-b border-gray-200 dark:border-gray-700">
                    <nav class="-mb-px flex space-x-1 overflow-x-auto" aria-label="Tabs">
                        @foreach (\App\Models\EmailTemplate::SYSTEM_TYPES as $key => $label)
                            <button type="button"
                                    @click="tab = '{{ $key }}'"
                                    :class="tab === '{{ $key }}'
                ...
                {{-- Template panels --}}
                @foreach (\App\Models\EmailTemplate::SYSTEM_TYPES as $type => $label)
                    @php
                    ...
                    <div x-show="tab === '{{ $type }}'" x-cloak class="pt-6 space-y-4">
                        ...
                        <div class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-800 dark:border-gray-700 p-6 space-y-4">
                            <form method="POST" action="{{ route('admin.settings.email-templates.save', $type) }}" id="admin-save-form-{{ $type }}">
                                @csrf
                            ...
                            <div class="flex items-center justify-between pt-2">
                                @if ($isCustom)
                                    <form method="POST" action="{{ route('admin.settings.email-templates.reset', $type) }}"
                                ...
                                <button type="submit" form="admin-save-form-{{ $type }}"
