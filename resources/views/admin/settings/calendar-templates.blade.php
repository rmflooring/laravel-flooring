    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Calendar Entry Templates</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Customise the title and body for calendar events created by the system.
                        These are admin-only and apply globally.
                    </p>
                </div>
            ...
            {{-- Tab nav --}}
            <div x-data="{ tab: '{{ request('tab', array_key_first(\App\Models\CalendarTemplate::TYPES)) }}' }">

                <div class="border-b border-gray-200 dark:border-gray-700">
                    <nav class="-mb-px flex space-x-1 overflow-x-auto" aria-label="Tabs">
                        @foreach (\App\Models\CalendarTemplate::TYPES as $key => $label)
                            <button type="button"
                                    @click="tab = '{{ $key }}'"
                                    :class="tab === '{{ $key }}'
                ...
                {{-- Template panels --}}
                @foreach (\App\Models\CalendarTemplate::TYPES as $type => $label)
                    @php
                        $saved          = $templates[$type] ?? null;
                        $default        = \App\Models\CalendarTemplate::DEFAULTS[$type] ?? ['title_template' => '', 'notes_template' => ''];
                        $titleTemplate  = $saved?->title_template ?? $default['title_template'];
                        $notesTemplate  = $saved?->notes_template ?? $default['notes_template'];
                        $tags           = \App\Models\CalendarTemplate::TAGS[$type] ?? [];
                        $isCustom       = (bool) $saved;
                    @endphp
                    ...
                    <div x-show="tab === '{{ $type }}'" x-cloak class="pt-6 space-y-4">
                        ...
                        <div class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-800 dark:border-gray-700 p-6 space-y-4">
                            <form method="POST" action="{{ route('admin.settings.calendar-templates.save', $type) }}" id="cal-save-form-{{ $type }}">
                                @csrf

                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Event Title</label>
                                        <input type="text" name="title_template" value="{{ old('title_template', $titleTemplate) }}"
                                        ...
                                        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">This becomes the calendar event subject in Microsoft 365.</p>
                                    </div>

                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Event Notes / Body</label>
                                        <textarea name="notes_template" rows="10"
                                                  class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2.5 text-sm font-mono dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('notes_template', $notesTemplate) }}</textarea>
                                        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">This becomes the calendar event description. Use line breaks to separate sections.</p>
                                    </div>
                                    ...
                                    <div class="p-3 bg-gray-50 rounded-lg dark:bg-gray-700/50">
                                        ...
                                        <p class="mt-2 text-xs text-gray-400 dark:text-gray-500">Click a tag to copy it. Hover for a description.</p>
                                    </div>
                                </div>
                            </form>
                            ...
                            <div class="flex items-center justify-between pt-2">
                                @if ($isCustom)
                                    <form method="POST" action="{{ route('admin.settings.calendar-templates.reset', $type) }}"
                                ...
                                <button type="submit" form="cal-save-form-{{ $type }}"
