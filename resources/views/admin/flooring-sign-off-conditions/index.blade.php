{{-- resources/views/admin/flooring-sign-off-conditions/index.blade.php --}}
<x-app-layout>
    <div class="max-w-screen-lg mx-auto px-4 sm:px-6 lg:px-8 py-6">

        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Sign-Off Conditions</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage selectable condition templates for Flooring Selection Sign-Offs.</p>
            </div>
        </div>
        ...
        <div class="mb-6 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <form method="POST" action="{{ route('admin.flooring-conditions.store') }}" class="p-4 space-y-3">
                @csrf
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between gap-3">
                        <label class="block text-xs font-medium text-gray-700 dark:text-gray-300">Body Text <span class="text-red-500">*</span></label>
                        <button type="button" onclick="insertSignatureTag(this.closest('div').querySelector('textarea'))"
                    ...
                    <textarea name="body" rows="4" required
                              placeholder="The full condition text that will appear on the sign-off document…"
                    ...
                    <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Place <span class="font-mono">@{{customer_signature}}</span> to position the e-signature at that location on the signed PDF.</p>
                </div>
                <div class="flex justify-end">
                    <button type="submit"
                            class="rounded-lg bg-emerald-600 px-5 py-2 text-sm font-medium text-white hover:bg-emerald-700 focus:outline-none focus:ring-4 focus:ring-emerald-300 dark:bg-emerald-600 dark:hover:bg-emerald-700 dark:focus:ring-emerald-800">
                        Add Condition
                    </button>
                </div>
            </form>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.flooring-conditions.edit', $condition->id) }}"
                                    ...
                                    <form method="POST"
                                          action="{{ route('admin.flooring-conditions.destroy', $condition->id) }}"
