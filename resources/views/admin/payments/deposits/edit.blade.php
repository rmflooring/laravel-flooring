    ...
    <div class="py-6">
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div>
                ...
                @if ($deposit->sale)
                    <p class="text-sm text-gray-500 mt-1">Sale #{{ $deposit->sale->sale_number }}</p>
                @endif
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                ...
                <form action="{{ route('admin.payments.deposits.update', $deposit) }}" method="POST" class="px-6 py-5 space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="date" name="payment_date"
                                   value="{{ old('payment_date', $deposit->payment_date?->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <select name="payment_method"
                            ...
                            @foreach ($paymentMethods as $value => $label)
                                <option value="{{ $value }}" @selected(old('payment_method', $deposit->payment_method) === $value)>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="reference_number"
                               value="{{ old('reference_number', $deposit->reference_number) }}"
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3"
                                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">{{ old('notes', $deposit->notes) }}</textarea>
                    </div>
            ...
            <p class="text-xs text-gray-400 text-center">
                Originally recorded by {{ $deposit->recordedBy?->name ?? 'unknown' }}
                on {{ $deposit->created_at?->format('M j, Y g:i A') }}
            </p>
