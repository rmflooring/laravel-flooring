    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Knowledge Entry</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Saving re-embeds the content — the old chunks are replaced.</p>
                </div>
            ...
            <form method="POST" action="{{ route('admin.knowledge.update', $entry) }}" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="content" id="content-field" rows="10" required
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                        placeholder="Separate distinct points with a blank line — each paragraph becomes its own searchable chunk.">{{ old('content', $entry->content) }}</textarea>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">PDF import replaces this box with the extracted text — review and clean it up (tables/columns often need fixing) before saving. Saving re-embeds all content, whether imported or not.</p>
                </div>
                ...
                <div>
                    ...
                    <textarea name="structured_data" rows="3"
                        ...
                        placeholder='{"markup_percent": 38}'>{{ old('structured_data', $entry->structured_data ? json_encode($entry->structured_data, JSON_PRETTY_PRINT) : '') }}</textarea>
                </div>
                        ...
                        @foreach ($roles as $role)
                            <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                <input type="checkbox" name="visible_to_roles[]" value="{{ $role }}"
                                    @checked(collect(old('visible_to_roles', $entry->visible_to_roles))->contains($role))
