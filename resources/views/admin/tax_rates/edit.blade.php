<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h1 class="text-3xl font-bold mb-6">Edit Tax Rate: {{ $taxRate->name }}</h1>

                    <form method="POST" action="{{ route('admin.tax_rates.update', $taxRate) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', $taxRate->name) }}" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="3" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $taxRate->description) }}</textarea>
                            ...
                            <div>
                                ...
                                <select name="tax_agency_id" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($agencies as $agency)
                                        <option value="{{ $agency->id }}" {{ old('tax_agency_id', $taxRate->tax_agency_id) == $agency->id ? 'selected' : '' }}>
                                            {{ $agency->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="md:col-span-2 flex items-center">
                                <input type="checkbox" id="collect_on_sales" name="collect_on_sales" value="1" {{ old('collect_on_sales', $taxRate->collect_on_sales) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <label for="collect_on_sales" class="ml-2 text-sm text-gray-700">I collect this on sales</label>
                            </div>

                            <!-- Sales Fields (conditional) -->
                            <div id="sales_fields" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 {{ old('collect_on_sales', $taxRate->collect_on_sales) ? '' : 'hidden' }}">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Tax on Sales (%)</label>
                                    <input type="number" step="0.0001" name="sales_rate" value="{{ old('sales_rate', $taxRate->sales_rate) }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                <div>
                                    ...
                                    <select name="sales_gl_account_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                        ...
                                        @foreach($glAccounts as $gl)
                                            <option value="{{ $gl->id }}" {{ old('sales_gl_account_id', $taxRate->sales_gl_account_id) == $gl->id ? 'selected' : '' }}>
                            ...
                            <div class="md:col-span-2 flex items-center">
                                <input type="checkbox" id="pay_on_purchases" name="pay_on_purchases" value="1" {{ old('pay_on_purchases', $taxRate->pay_on_purchases) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <label for="pay_on_purchases" class="ml-2 text-sm text-gray-700">I pay this on purchases</label>
                            </div>

                            <!-- Purchases Fields (conditional) -->
                            <div id="purchases_fields" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 {{ old('pay_on_purchases', $taxRate->pay_on_purchases) ? '' : 'hidden' }}">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Tax on Purchases (%)</label>
                                    <input type="number" step="0.0001" name="purchase_rate" value="{{ old('purchase_rate', $taxRate->purchase_rate) }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                <div>
                                    ...
                                    <select name="purchase_gl_account_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                        <option value="">-- Select GL Account --</option>
                                        @foreach($glAccounts as $gl)
                                            <option value="{{ $gl->id }}" {{ old('purchase_gl_account_id', $taxRate->purchase_gl_account_id) == $gl->id ? 'selected' : '' }}>
                                                {{ $gl->account_number }} - {{ $gl->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="md:col-span-2 flex items-center">
                                <input type="checkbox" id="show_on_return_line" name="show_on_return_line" value="1" {{ old('show_on_return_line', $taxRate->show_on_return_line) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
    ...
    <!-- JavaScript for conditional fields (same as create) -->
    <script>
