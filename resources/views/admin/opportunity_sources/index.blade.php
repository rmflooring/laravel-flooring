    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Job Sources</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage the source categories used to track where opportunities come from (Retail, Referral, Website, etc.) for marketing reporting.</p>
                </div>
            </div>
            ...
            <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                <h2 class="text-base font-semibold text-gray-900 dark:text-white mb-4">Add New Source</h2>
                <form action="{{ route('admin.opportunity_sources.store') }}" method="POST" class="flex items-end gap-3">
                    @csrf
                    <div class="flex-1">
                        <label for="name" class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Source Name</label>
                        <input
                            type="text"
                            id="name"
                            name="name"
                            value="{{ old('name') }}"
                            placeholder="e.g. Retail, Referral, Website, Repeat Customer..."
            ...
            {{-- Filters --}}
            <form method="GET" action="{{ route('admin.opportunity_sources.index') }}" class="flex items-center gap-3">
                <input
                    type="text"
                    name="search"
                    value="{{ $search }}"
                ...
                @if ($search || $showInactive)
                    <a href="{{ route('admin.opportunity_sources.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
                @endif
            </form>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th class="px-6 py-3">Opportunities</th>
                        ...
                        @forelse ($sources as $source)
                            <tr class="bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 text-gray-500">
                                    {{ $source->opportunities_count }}
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('admin.opportunity_sources.edit', $source) }}"
                                       class="font-medium text-blue-600 hover:underline dark:text-blue-400 mr-4">
                                        Edit
                                    </a>
                                    @if ($source->opportunities_count === 0)
                                        <form action="{{ route('admin.opportunity_sources.destroy', $source) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                    onclick="return confirm('Delete source &quot;{{ addslashes($source->name) }}&quot;?')"
