    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                ...
                <span>/</span>
                <span class="font-medium text-gray-900 dark:text-white">New Credit Memo</span>
            </nav>
            ...
            @if ($rtv)
                <div class="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800 dark:border-green-800 dark:bg-green-900/20 dark:text-green-300">
                    ...
                    Pre-filled from RTV <a href="{{ route('pages.inventory.rtv.show', $rtv) }}" class="font-semibold underline hover:no-underline">{{ $rtv->return_number }}</a>
                </div>
            @endif

            <form method="POST" action="{{ route('admin.vendor-credits.store') }}">
                @csrf

                @if ($rtv)
                    <input type="hidden" name="inventory_return_id" value="{{ $rtv->id }}">
                @endif
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    {{-- Header --}}
                    <div class="px-6 py-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">New Vendor Credit Memo</h2>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Record a credit received from a vendor. This reduces your AP payables balance for that vendor.</p>
                    </div>
                    ...
                    <div class="px-6 py-5 space-y-5">
                        ...
                        <div>
                            ...
                            <select name="vendor_id" id="vendor_id" required
                                ...
                                @foreach ($vendors as $v)
                                    <option value="{{ $v->id }}"
                                        {{ (old('vendor_id', $rtv?->vendor_id) == $v->id) ? 'selected' : '' }}>
                                        {{ $v->company_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <input type="text" name="reference_number" id="reference_number"
                                       value="{{ old('reference_number', $rtv?->vendor_reference) }}"
                        ...
                        <div>
                            ...
                            @php
                                $rtvSubtotal = $rtv ? $rtv->items->sum('credit_received') : null;
                                if (!$rtvSubtotal && $rtv) {
                                    $rtvSubtotal = $rtv->items->sum('line_total');
                                }
                            @endphp
                            <div class="relative">
                                ...
                                <input type="number" name="subtotal" id="subtotal" required
                                       min="0" step="0.01"
                                       value="{{ old('subtotal', $rtvSubtotal ? number_format((float)$rtvSubtotal, 2, '.', '') : '') }}"
                        ...
                        <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4 space-y-4">
                            ...
                            {{-- Tax group quick-fill --}}
                            <div id="tax-group-row" class="{{ old('tax_manual') ? 'hidden' : '' }}">
                            ...
                            <div id="manual-amounts-row" class="{{ old('tax_manual') ? '' : 'hidden' }}">
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="number" name="gst_amount_override" id="gst_amount_override"
                                                   min="0" step="0.01"
                                                   value="{{ old('gst_amount_override', 0) }}"
                                                   class="w-full pl-7 rounded-lg border-gray-300 text-sm shadow-sm focus:border-green-500 focus:ring-green-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                                   oninput="recalcTax()">
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="number" name="pst_amount_override" id="pst_amount_override"
                                                   min="0" step="0.01"
                                                   value="{{ old('pst_amount_override', 0) }}"
                                                   class="w-full pl-7 rounded-lg border-gray-300 text-sm shadow-sm focus:border-green-500 focus:ring-green-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                                   oninput="recalcTax()">
                                        </div>
                                    </div>
                            ...
                            <div class="rounded-md bg-gray-50 dark:bg-gray-700/30 p-3 text-sm space-y-1">
                                ...
                                <div class="flex justify-between text-gray-600 dark:text-gray-400">
                                    <span>GST</span>
                                    <span id="display-gst">$0.00</span>
                                </div>
                                <div class="flex justify-between text-gray-600 dark:text-gray-400">
                                    <span>PST</span>
                                    <span id="display-pst">$0.00</span>
                                </div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                      placeholder="Reason for credit, context…"
                    ...
                    {{-- Footer --}}
                    <div class="flex items-center justify-end gap-3 px-6 py-4 bg-gray-50 dark:bg-gray-700/30 rounded-b-lg">
