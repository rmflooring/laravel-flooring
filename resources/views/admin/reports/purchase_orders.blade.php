                ...
                <div class="p-4 text-gray-900">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <h1 class="text-3xl font-bold">Purchase Orders Report</h1>
                        </div>
                    ...
                    {{-- Filters --}}
                    <form method="GET" action="{{ route('admin.reports.purchaseOrders') }}" class="mb-6">
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                                ...
                                <div>
                                    ...
                                    <select name="vendor_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            ...
                            <tbody>
                                @forelse($purchaseOrders as $po)
                                    @php
                                        ...
                                        $statusColor = $statusColors[$po->status] ?? 'bg-gray-100 text-gray-700';

                                        $deliveryOverdue = $po->expected_delivery_date
                                            && $po->expected_delivery_date->isPast()
                                            && !in_array($po->status, ['delivered', 'cancelled']);
                                    @endphp
                                    <tr class="bg-white border-b hover:bg-gray-50 {{ $deliveryOverdue ? 'bg-yellow-50' : '' }}">
                                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                            <a href="{{ route('pages.purchase-orders.show', $po) }}" class="text-blue-600 hover:underline">{{ $po->po_number }}</a>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            @if($po->sale)
                                                <a href="{{ route('pages.sales.show', $po->sale) }}" class="text-blue-600 hover:underline">#{{ $po->sale->sale_number }}</a>
                                            @else
                                                <span class="text-gray-400">-</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3">{{ $po->sale?->customer_name ?? '-' }}</td>
                                        <td class="px-4 py-3 font-medium text-gray-900">{{ $po->vendor?->name ?? '-' }}</td>
                                        <td class="px-4 py-3">
                                            <span class="px-2 py-0.5 rounded text-xs font-medium {{ $statusColor }}">
                                                {{ ucwords($po->status) }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-3 text-gray-600">{{ $po->fulfillment_label ?? '-' }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap {{ $deliveryOverdue ? 'text-yellow-700 font-medium' : '' }}">
                                            {{ $po->expected_delivery_date?->format('M j, Y') ?? '-' }}
                                            @if($deliveryOverdue)
                                                <span class="text-xs text-yellow-600">(overdue)</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3 text-gray-500 whitespace-nowrap">{{ $po->created_at->format('M j, Y') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="8" class="px-4 py-8 text-center text-gray-400">No purchase orders found matching your filters.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
