                    ...
                    <form method="POST" action="{{ route('admin.vendor_reps.update', $vendorRep) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="vendor_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach ($vendors as $vendor)
                                        <option value="{{ $vendor->id }}" {{ old('vendor_id', $currentVendorId) == $vendor->id ? 'selected' : '' }}>
                                            {{ $vendor->company_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="email" name="email" value="{{ old('email', $vendorRep->email) }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 email-input">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Phone</label>
                                <input type="text" name="phone" value="{{ old('phone', $vendorRep->phone) }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 phone-input">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Mobile</label>
                                <input type="text" name="mobile" value="{{ old('mobile', $vendorRep->mobile) }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 phone-input">
                            </div>
                        </div>
