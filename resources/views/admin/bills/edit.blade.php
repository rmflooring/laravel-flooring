<x-app-layout>
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Invoice #{{ $bill->reference_number }} · {{ $bill->payee_name }}
                    </p>
                </div>
            ...
            <form method="POST" action="{{ route('admin.bills.update', $bill) }}" x-data="billEditForm()">
                @csrf @method('PUT')
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <select name="vendor_id"
                                ...
                                @foreach ($vendors as $vendor)
                                    <option value="{{ $vendor->id }}" @selected(old('vendor_id', $bill->vendor_id) == $vendor->id)>{{ $vendor->company_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @else
                        <div>
                            <label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Installer</label>
                            <select name="installer_id"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                <option value="">— Select installer —</option>
                                @foreach ($installers as $installer)
                                    <option value="{{ $installer->id }}" @selected(old('installer_id', $bill->installer_id) == $installer->id)>{{ $installer->company_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            <input type="text" name="reference_number" value="{{ old('reference_number', $bill->reference_number) }}"
                        ...
                        <div>
                            ...
                            <select name="status"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                @foreach (['draft' => 'Draft', 'pending' => 'Pending', 'approved' => 'Approved', 'overdue' => 'Overdue'] as $val => $label)
                                    <option value="{{ $val }}" @selected(old('status', $bill->status) === $val)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="bill_date" id="bill_date"
                                value="{{ old('bill_date', $bill->bill_date->toDateString()) }}"
                                x-model="billDate" @change="updateDueDateFromTerm"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="payment_term_id" id="payment_term_id"
                                x-model="termId" @change="updateDueDateFromTerm"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                <option value="">— None —</option>
                                @foreach ($paymentTerms as $term)
                                    <option value="{{ $term->id }}" data-days="{{ $term->days }}"
                                        @selected(old('payment_term_id', $bill->payment_term_id) == $term->id)>
                                        {{ $term->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date"
                                value="{{ old('due_date', $bill->due_date?->toDateString()) }}"
                                x-model="dueDate"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('notes', $bill->notes) }}</textarea>
                    </div>
                </div>
                                    ...
                                    <tr class="border-t border-gray-100 dark:border-gray-700">
                                        <td class="px-4 py-2">
                                            ...
                                            <input type="text" :name="`items[${index}][item_name]`" x-model="row.item_name"
                                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="number" :name="`items[${index}][quantity]`" x-model.number="row.quantity"
                                                @input="recalcRow(row); recalculate()" step="any" min="0"
                                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                                        </td>
                                        <td class="px-4 py-2">
                                        ...
                                        <td class="px-4 py-2 text-center">
                                            <button type="button" @click="removeRow(index)"
                                                class="text-red-500 hover:text-red-700 dark:text-red-400" x-show="rows.length > 1">
                                                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <div class="flex flex-col items-end gap-1 text-sm text-gray-700 dark:text-gray-300">
                            ...
                            <div class="flex justify-between w-72 text-green-700 dark:text-green-400" x-show="grossEarlyPaymentTotal > 0">
                                ...
                                <span class="font-medium">−$<span x-text="grossEarlyPaymentTotal.toFixed(2)"></span></span>
                            </div>
...
@php
    $initialRows = $bill->items->filter(fn($item) => is_null($item->charge_type))
        ->values()
        ->map(fn ($item, $i) => [
            'key'                    => $i,
            'id'                     => $item->id,
    ...
    $initialChargeRows = $bill->items->filter(fn($item) => ! is_null($item->charge_type))
        ->values()
        ->map(fn ($item, $i) => [
            'key'         => $i,
            'id'          => $item->id,
...
function billEditForm() {
    ...
    const initialChargeRows = @json($initialChargeRows);
    ...
    return {
        ...
        subtotal: 0, chargeTotal: 0, grossChargesTotal: 0, grossCreditsTotal: 0, grossEarlyPaymentTotal: 0, gstAmount: 0, pstAmount: 0, grandTotal: 0,
        ...
        removeRow(index) {
            if (this.rows.length > 1) { this.rows.splice(index, 1); this.recalculate(); }
        },

        addCharge() {
            this.chargeRows.push({ key: this.chargeKey++, id: null, charge_type: 'fuel', item_name: chargeTypeLabels['fuel'], unit_cost: 0, line_total: 0 });
        },
