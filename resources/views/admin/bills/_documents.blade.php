{{-- resources/views/admin/bills/_documents.blade.php --}}
{{-- Usage: @include('admin.bills._documents', ['bill' => $bill]) --}}
<div class="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"
     ...
     @drop.prevent="
         dragging = false;
         const input = document.getElementById('bill-doc-upload-{{ $bill->id }}');
         const form  = document.getElementById('bill-doc-upload-form-{{ $bill->id }}');
    ...
    <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
        ...
        @can('edit bills')
        <label for="bill-doc-upload-{{ $bill->id }}"
        ...
        <form id="bill-doc-upload-form-{{ $bill->id }}"
              method="POST"
              action="{{ route('admin.bills.documents.store', $bill) }}"
              enctype="multipart/form-data"
              class="hidden">
            @csrf
            <input type="file"
                   id="bill-doc-upload-{{ $bill->id }}"
                   name="files[]"
                   multiple
                   accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx,.xls,.xlsx,.csv,.txt"
                   class="hidden"
                   onchange="document.getElementById('bill-doc-upload-form-{{ $bill->id }}').submit()">
        </form>
        @endcan
    </div>
    ...
    <div class="divide-y divide-gray-100 dark:divide-gray-700">
        @forelse ($bill->documents as $doc)
            <div class="flex items-center gap-3 px-6 py-3">
                ...
                <div class="min-w-0 flex-1">
                    <a href="{{ route('admin.bills.documents.download', [$bill, $doc]) }}"
                       target="_blank"
                ...
                <div class="flex shrink-0 items-center gap-2">
                    ...
                    @can('edit bills')
                    <form method="POST"
                          action="{{ route('admin.bills.documents.destroy', [$bill, $doc]) }}"
        ...
        @empty
            <div class="px-6 py-8 text-center">
                ...
                <p class="text-sm text-gray-400 dark:text-gray-500">No documents uploaded yet.</p>
                @can('edit bills')
                <label for="bill-doc-upload-{{ $bill->id }}"
