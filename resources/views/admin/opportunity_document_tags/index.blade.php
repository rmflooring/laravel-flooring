    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Document Tags</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage tags used to categorize opportunity media and documents.</p>
                </div>
            </div>
            ...
            <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                <h2 class="text-base font-semibold text-gray-900 dark:text-white mb-4">Add New Tag</h2>
                <form action="{{ route('admin.opportunity_document_tags.store') }}" method="POST" class="flex items-end gap-3">
                    @csrf
                    <div class="flex-1">
                        <label for="name" class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Tag Name</label>
                        <input
                            type="text"
                            id="name"
                            name="name"
                            value="{{ old('name') }}"
                            placeholder="e.g. Before, After, Damage, Kitchen..."
            ...
            {{-- Filters --}}
            <form method="GET" action="{{ route('admin.opportunity_document_tags.index') }}" class="flex items-center gap-3">
                <input
                    type="text"
                    name="search"
                    value="{{ $search }}"
                ...
                @if ($search || $showInactive)
                    <a href="{{ route('admin.opportunity_document_tags.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
                @endif
            </form>
                        ...
                        @forelse ($tags as $tag)
                            <tr class="bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('admin.opportunity_document_tags.edit', $tag) }}"
                                       class="font-medium text-blue-600 hover:underline dark:text-blue-400 mr-4">
                                        Edit
                                    </a>
                                    @if ($tag->documents_count === 0)
                                        <form action="{{ route('admin.opportunity_document_tags.destroy', $tag) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                    onclick="return confirm('Delete tag &quot;{{ addslashes($tag->name) }}&quot;?')"
