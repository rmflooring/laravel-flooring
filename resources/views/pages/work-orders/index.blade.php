{{-- resources/views/pages/work-orders/index.blade.php --}}
<x-app-layout>
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Search and filter all work orders.</p>
                </div>
            </div>
            ...
            {{-- Filters --}}
            <form method="GET" action="{{ route('pages.work-orders.index') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div class="md:col-span-12 flex flex-wrap items-center gap-2">
                        ...
                        <a href="{{ route('pages.work-orders.index') }}"
                        ...
                        <div class="ml-auto text-sm text-gray-600">
                            Showing <span class="font-semibold">{{ $workOrders->firstItem() ?? 0 }}</span>
                            to <span class="font-semibold">{{ $workOrders->lastItem() ?? 0 }}</span>
                            of <span class="font-semibold">{{ $workOrders->total() }}</span>
                        </div>
                    </div>
                </div>
            </form>
                    ...
                    <table class="w-full text-sm text-left text-gray-700">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b">
                            <tr>
                                ...
                                <th class="px-6 py-3">Installer</th>
                        ...
                        <tbody>
                            @forelse ($workOrders as $wo)
                                ...
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($wo->sale)
                                            <a href="{{ route('pages.sales.show', $wo->sale) }}"
                                    ...
                                    <td class="px-6 py-4 text-gray-600">
                                        @if ($wo->scheduled_date)
                                            {{ \Carbon\Carbon::parse($wo->scheduled_date)->format('Y-m-d') }}
                                            @if ($wo->scheduled_time)
                                                <span class="text-xs text-gray-500">{{ $wo->scheduled_time }}</span>
                                            @endif
                                        @else
                                            <span class="text-gray-400">—</span>
                                        @endif
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        @if ($wo->sale)
                                            <a href="{{ route('pages.sales.work-orders.show', [$wo->sale, $wo]) }}"
