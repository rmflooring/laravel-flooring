    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="POST" action="{{ route('pages.samples.checkout.store') }}"
                  x-data="checkoutBuilder(@js($samples->map(fn ($s) => [
                        'id' => $s->id,
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-6 space-y-4 mb-6">
                    ...
                    <div x-show="type === 'customer'" x-cloak class="space-y-4 pt-2 border-t border-gray-100 dark:border-gray-700">
                        <div>
                            <label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Select Existing Customer</label>
                            <select name="customer_id" x-model="customerId"
                                    @change="const opt = $event.target.options[$event.target.selectedIndex]; customerName = opt.dataset.name || ''; customerPhone = opt.dataset.phone || ''; customerEmail = opt.dataset.email || '';"
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-6 space-y-4 mb-6">
                    <h2 class="text-base font-semibold text-gray-900 dark:text-white">Samples</h2>

                    <div class="relative">
                        <input type="text" x-model="query" @input.debounce.300ms="search()"
                               placeholder="Search by sample #, style, color, or manufacturer…"
                               autocomplete="off"
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:text-white dark:border-gray-600">
                        ...
                        <div x-show="query.length > 0 && results.length > 0" x-cloak
                            ...
                            <template x-for="result in results" :key="result.id">
                                <button type="button" @click="addSample(result)"
                                    ...
                                    <span>
                                        <span class="font-medium text-gray-900 dark:text-white" x-text="result.sample_id"></span>
                                        <span class="text-gray-500 dark:text-gray-400" x-text="' — ' + (result.manufacturer || '') + ' ' + (result.style_name || '')"></span>
                                    </span>
                                    <span class="text-xs text-gray-400 flex-shrink-0 ml-2" x-text="result.available_qty + ' available'"></span>
                                </button>
                            </template>
                        </div>
                        <div x-show="query.length > 0 && !searching && results.length === 0" x-cloak
                            ...
                            No available samples match "<span x-text="query"></span>".
                        </div>
                    </div>

                    {{-- Cart --}}
                    <div x-show="cart.length === 0" x-cloak class="text-sm text-gray-400 py-4 text-center">
                        No samples added yet — search above to add some.
                    </div>
                            ...
                            <template x-for="item in cart" :key="item.id">
                                <tr>
                                    <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">
                                        <span x-text="item.sample_id"></span>
                                        <div class="text-xs font-normal text-gray-500 dark:text-gray-400" x-text="(item.manufacturer || '') + ' ' + (item.style_name || '')"></div>
                                        <input type="hidden" name="sample_ids[]" :value="item.id">
                                    </td>
                                    <td class="px-4 py-3 text-right text-gray-500" x-text="item.available_qty"></td>
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <button type="button" @click="removeSample(item.id)"
                ...
                <div class="flex items-center gap-3">
                    <button type="submit" :disabled="!isValid"
                        ...
                        <span x-text="cart.length > 0 ? 'Check Out ' + cart.length + ' Sample' + (cart.length === 1 ? '' : 's') : 'Check Out'"></span>
                    </button>
                    <a href="{{ route('pages.samples.index') }}"
        ...
        Alpine.data('checkoutBuilder', (initialCart) => ({
            ...
            search() {
                ...
                fetch('{{ route('pages.samples.checkout.search') }}?q=' + encodeURIComponent(this.query))
                    .then(r => r.json())
                    .then(data => {
                        const inCart = new Set(this.cart.map(c => c.id));
                        this.results = data.filter(r => ! inCart.has(r.id));
            ...
            addSample(sample) {
                if (this.cart.some(c => c.id === sample.id)) return;
                this.cart.push({ ...sample, qty: 1 });
                this.query = '';
                this.results = [];
            },
