                    ...
                    <div class="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                @elseif (in_array($pickTicket->status, ['delivered', 'partially_delivered']))
                                    @if ($pickTicket->status === 'partially_delivered')
                                        <button type="button"
                                                onclick="document.getElementById('pt-deliver-modal').classList.remove('hidden')"
                                                class="inline-flex items-center rounded-md bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-700">
                                            Record Delivery
                        ...
                        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                            <thead>
                                <tr class="bg-gray-50 dark:bg-gray-700/50">
                                    ...
                                    @if ($hasReturns)
                                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Returned</th>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @forelse ($pickTicket->items as $ptItem)
                                    @php
                                        ...
                                        $remaining = max(0, $ordered - $atSite);
                                    ...
                                    <tr class="{{ $fullyDone ? 'opacity-60' : '' }}">
                                        ...
                                        @if ($hasReturns)
                                            <td class="px-4 py-3 text-right text-sm font-medium {{ $returned > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400' }}">
                                                {{ $returned > 0 ? rtrim(rtrim(number_format($returned, 2), '0'), '.') : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-right text-sm font-medium {{ $atSite > 0 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400' }}">
