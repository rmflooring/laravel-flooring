...
<a href="{{ route('pages.opportunities.show', $sale->opportunity_id) }}"
   ...
   class="inline-flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg hover:bg-indigo-100 focus:outline-none focus:ring-4 focus:ring-indigo-200">
    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      ...
      <td class="px-3 py-2">
        @php
          ...
          $qtyStyle = match($poStatus) {
              'delivered' => 'background-color:#0f766e; border-color:#0f766e; color:#ffffff;',
              'received'  => 'background-color:#bbf7d0; border-color:#4ade80; color:#166534;',
              'ordered'   => 'background-color:#fef9c3; border-color:#fde047; color:#854d0e;',
    ...
    class="material-cost-total-input">
      ...
      <td class="px-3 py-2">
        <div class="rich-notes-field w-44 bg-gray-50 border border-gray-300 rounded-lg p-2"
          ...
          style="min-height:2rem;cursor:text;">{!! old("rooms.$roomIndex.materials.$i.po_notes", $item->po_notes) !!}</div>
        <input type="hidden"
    ...
    <input type="text"
      ...
      <td class="px-3 py-2">
        <div class="rich-notes-field w-56 bg-gray-50 border border-gray-300 rounded-lg p-2"
          ...
          style="min-height:2rem;cursor:text;">{!! old("rooms.$roomIndex.labour.$i.notes", $item->notes ?? $item->item_notes ?? '') !!}</div>
        <input type="hidden"
...
<input type="hidden"
  ...
  value="0">
    ...
    <td class="px-3 py-2">
      ...
      <input type="hidden" name="rooms[__ROOM_INDEX__][freight][__ITEM_INDEX__][cost_total]" class="freight-cost-total-input" value="0">
    </td>
...
<div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-0.5">WOs raised against this sale.</p>
                ...
                <table class="w-full text-left text-sm text-gray-700">
                    <thead class="text-xs text-gray-500 bg-gray-50 border-b border-gray-100 uppercase">
                        <tr>
                            ...
                            <th class="px-5 py-3">Installer</th>
                            <th class="px-5 py-3">Status</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($sale->workOrders as $wo)
                            <tr class="hover:bg-gray-50">
                                <td class="px-5 py-3 font-medium text-gray-900">{{ $wo->wo_number }}</td>
                                <td class="px-5 py-3">{{ $wo->installer->company_name ?? '—' }}</td>
                                <td class="px-5 py-3">
                                ...
                                <td class="px-5 py-3 text-gray-500">
                                    ...
                                    @if($wo->scheduled_time)
                                        <span class="text-gray-400"> {{ \Carbon\Carbon::parse($wo->scheduled_time)->format('g:i A') }}</span>
                                    @endif
                                </td>
                                <td class="px-5 py-3 font-medium">${{ number_format($wo->grand_total, 2) }}</td>
                                <td class="px-5 py-3">
                                    <a href="{{ route('pages.sales.work-orders.show', [$sale, $wo]) }}"
                                       class="text-sm font-medium text-blue-600 hover:underline">View</a>
                                    @can('edit work orders')
                                    &nbsp;·&nbsp;
                                    <a href="{{ route('pages.sales.work-orders.edit', [$sale, $wo]) }}"
                                       class="text-sm font-medium text-blue-600 hover:underline">Edit</a>
                        ...
                        <select name="payer_type" class="w-full text-sm border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                            ...
                            @foreach($depositPayerOptions as $opt)
                                <option value="{{ $opt['type'] }}" data-customer-id="{{ $opt['customer']->id }}">
                                    {{ $opt['customer']->company_name ?: $opt['customer']->name ?: '(unnamed)' }}
...
<div x-data="{
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl" @click.outside="open = false">
        ...
        <form method="POST" action="{{ route('pages.sales.send-email', $sale) }}" enctype="multipart/form-data">
            @csrf
