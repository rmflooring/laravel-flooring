...
class ProcessAgentTask implements ShouldQueue
{
    ...
    private const SYSTEM_PROMPT = <<<'TEXT'
        ...
        have direct database or shell access. Only call attach_images when the email
        is clearly about the opportunity already resolved for this task and contains
        photo attachments. Only call attach_document when the email contains a
        non-photo document attachment (PDF, Word doc, or scanned document) such as a
        scope of work, contract, or insurance certificate, and is clearly about the
        opportunity already resolved for this task. If you cannot confidently
        determine what's being asked, or the email doesn't relate to the resolved
        opportunity, call request_clarification with a specific question. If the
        email is not an actionable request at all (spam, newsletter, unrelated
        forward), call no_actionable_intent. Call exactly one tool to conclude the
        task.
        TEXT;
    ...
    public function handle(
        ...
        for ($i = 0; $i < self::MAX_TOOL_ITERATIONS; $i++) {
            ...
            foreach ($toolUses as $toolUse) {
                [$toolResult, $maybeTerminal] = $this->dispatchTool($task, $attachImages, $attachDocument, $toolUse);
        ...
        if ($result === null) {
            ...
            $result = ['status' => 'pending_clarification', 'summary' => 'Could not resolve the request automatically.'];
    ...
    /**
     * Minimal deterministic job-number lookup (Module 1 stand-in for the full
     * fuzzy find_opportunity tool). Job numbers look like "26-0001".
     */
            ...
            switch ($name) {
                ...
                case 'request_clarification':
                    ...
                    return [
                        ...
                        ['status' => 'pending_clarification', 'summary' => $question],
                    ];
                ...
                case 'no_actionable_intent':
                    ...
                    return [
                        ...
                        ['status' => 'ignored', 'summary' => $summary],
                    ];
    ...
    private function finalizeFromText(AgentTask $task, array $content): array
    {
        ...
        return ['status' => 'pending_clarification', 'summary' => $summary];
    }
