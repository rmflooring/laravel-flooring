...
$data['tax_rate_percent'] = $groupPercent;
        ...
        $estimate = DB::transaction(function () use ($data, $rooms) {
            ...
            $estimate = Estimate::create([
                ...
                'homeowner_mobile'   => $data['homeowner_mobile'] ?? null,
...
if ($taxGroupId) {
    ...
    'db_email'       => $estimate->homeowner_email,
        ...
        $estimate->forceFill([
            'customer_name'      => array_key_exists('parent_customer_name', $data) ? $data['parent_customer_name'] : $estimate->customer_name,
            'customer_po'        => array_key_exists('customer_po', $data) ? ($data['customer_po'] ?? null) : $estimate->customer_po,
            'pm_name'            => array_key_exists('pm_name', $data) ? $data['pm_name'] : $estimate->pm_name,
			'salesperson_1_employee_id' => $data['salesperson_1_employee_id'] ?? null,
			'salesperson_2_employee_id' => $data['salesperson_2_employee_id'] ?? null,
            'job_no'             => array_key_exists('job_number', $data) ? $data['job_number'] : $estimate->job_no,
            'job_name'           => array_key_exists('job_name', $data) ? $data['job_name'] : $estimate->job_name,
            'job_address'        => array_key_exists('job_address', $data) ? $data['job_address'] : $estimate->job_address,
			'homeowner_name'   => array_key_exists('homeowner_name', $data) ? $data['homeowner_name'] : $estimate->homeowner_name,
			'homeowner_phone'  => array_key_exists('homeowner_phone', $data) ? $data['homeowner_phone'] : $estimate->homeowner_phone,
			'homeowner_mobile' => array_key_exists('homeowner_mobile', $data) ? $data['homeowner_mobile'] : $estimate->homeowner_mobile,
			'homeowner_email'  => array_key_exists('homeowner_email', $data) ? $data['homeowner_email'] : $estimate->homeowner_email,
            'notes'              => array_key_exists('notes', $data) ? $data['notes'] : $estimate->notes,
